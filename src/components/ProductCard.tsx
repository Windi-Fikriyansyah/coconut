...
const ProductCard: React.FC<ProductCardProps> = ({ slug, title, description, image, tags }) => {
    ...
    return (
        ...
        >
            <Link href={`/products/${slug}`} className="relative h-48 overflow-hidden block">
                <Image
                    src={image}
                    alt={title}
                    fill
            ...
            <div className="p-8 flex-1 flex flex-col">
                <Link href={`/products/${slug}`}>
                    <h3 className="text-xl font-bold text-coco-forest mb-3 group-hover:text-coco-gold transition-colors">{title}</h3>
                </Link>
                <p className="text-coco-forest/60 text-sm leading-relaxed mb-6 flex-1">
                    {description}
                </p>
                <div className="flex flex-wrap items-center gap-4 mt-auto">
                    <Link href={`/products/${slug}`} className="flex items-center gap-2 text-coco-forest font-bold text-xs hover:text-coco-gold transition-colors white-nowrap">
                        View Detail
                        <MoveRight className="w-4 h-4" />
                    </Link>
                    <a
                        href={`mailto:${emailTo}?subject=${emailSubject}`}
                        className="bg-coco-gold/10 text-coco-gold px-4 py-2 rounded-full font-bold text-xs hover:bg-coco-gold hover:text-coco-forest transition-all"
                    >
                        Get Quote
                    </a>
                </div>
